...
| 方面 | 机制 |
|------|------|
| 空闲机制 | 主动轮询循环（sleep 1s，检查收件箱/看板） |
| 自动认领 | 代码驱动（idle 循环中的 `_scan_unclaimed_tasks()`） |
| 身份注入 | `auto_compact` 后显式重新注入 |

主动轮询让空闲/唤醒模式可观察、可调试。也可以通过 turn 级事件注入达到相同效果。
...
v6: 多个 Agent, 有看板
v7: 多个 Agent, 能并行
v8: 多个 Agent, 能通信 (v8a 基础 -> v8b 通信 -> v8c 协调)
v9: 多个 Agent, 能自治
```
