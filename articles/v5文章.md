...
def microcompact(self, messages: list) -> list:
    ...
    if estimated_savings >= MIN_SAVINGS:  # MIN_SAVINGS = 20000
...
- **只清理特定工具**。COMPACTABLE_TOOLS 包含全部 8 种工具（bash、read_file、write_file、edit_file、glob、grep、list_dir、notebook_edit），覆盖了所有可能产生大输出的工具。只有超过 1000 token 的输出才会被清理，小输出自然跳过。
- **保留最近 3 个**。最近的工具输出往往是当前工作的上下文，不能丢。
- **MIN_SAVINGS 门槛**。只有当预估可节省 token 数达到 20000 时才执行清理，避免在收益太小时频繁修改消息。
...
这里有一个极其重要的缓存设计：**摘要注入到对话历史（user message），而不是修改系统提示词**。auto_compact 会用 [summary, ack, ...restored_files] 完全替换原有的所有消息，没有"保留最近 N 条"的行为。
...
def handle_large_output(self, output: str) -> str:
    ...
    filename = f"output_{int(time.time())}.output"
