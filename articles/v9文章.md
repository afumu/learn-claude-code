...
def _idle_phase(self, teammate: Teammate, sub_messages: list) -> str:
    ...
    # 2. 计算轮询次数: 60s / 1s = 60 次
    polls = IDLE_TIMEOUT // IDLE_POLL_INTERVAL    # 60 // 1 = 60
...
```python
IDLE_POLL_INTERVAL = 1     # 每次轮询间隔 1 秒
...
| v8 | 团队通信 | ~1402 | Teammate 消息协议 (v8a/v8b/v8c) |
